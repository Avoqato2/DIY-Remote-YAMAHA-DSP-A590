...
//----------------------------------------------------------------
//----------------------------------------------------------------
// Channel Menu and Effect Menu Strings
...
String effect_str_arr[NUM_EFFECTS] = {
  "Back","PROLOGIC", "ENHANCED", "CONCERT HALL", "CONCERT VIDEO",
...
void handle_menu_navigation(String menu[], int NUM_ITEMS){
  ...
  if (digitalRead(rotary_SW) == LOW) {
      ...
      }else if(is_in_channelmenu || is_in_effectmenu){ // If we are in the channel or effect menu, check if the "Back" item is selected to return to the start menu
        if(current_selected_item == 0){
